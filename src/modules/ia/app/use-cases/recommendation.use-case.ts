  ...
  async getRecommendationBasedOnboarding(
    ...
    try {
      ...
      const preferredTypes: string[] =
        userPreferences?.placeTypes?.map((pt) => pt.placeType.name) || [];
      const maxPrice: number = userPreferences
        ? Number(userPreferences.maxPrice)
        : 200;
      ...
      const payload: RecommendationRequestPayloadDto = {
        ...
        Restaurant: restaurants.map((restaurant) => {
          const plainRestaurant: PlainRestaurant = restaurant.get({
            plain: true,
          });
          return {
      ...
      const response =
        await this.iaService.sendRecommendationBasedOnboarding(payload);
      ...
      if (
        ...
        const recommendedRestaurants =
          await this.restaurantRepository.findByIds(fallbackIds);
      ...
      const recommendedRestaurantIds = response.restaurants.map(
        (item) => item.restaurantId,
      );

      const recommendedRestaurants = await this.restaurantRepository.findByIds(
        recommendedRestaurantIds,
      );

      if (!recommendedRestaurants || recommendedRestaurants.length === 0) {
        throw new InternalServerErrorException(
          'Recommended restaurants not found in database',
        );
      }
