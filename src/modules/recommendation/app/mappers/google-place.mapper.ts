...
interface GoogleOpeningHours {
  periods?: RestaurantOpeningHours['periods'];
...
export class GooglePlaceMapper {
  ...
  private static mapOpeningHours(
    openingHours?: GoogleOpeningHours,
  ): RestaurantOpeningHours | undefined {
    if (!openingHours?.periods?.length && !openingHours?.weekdayDescriptions) {
      return undefined;
    }
